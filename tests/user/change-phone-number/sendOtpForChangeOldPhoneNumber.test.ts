...
describe("[POST] /api/user/send-otp-for-change-old-phone-number", () => {
    ...
    describe("Given all fields", () => {
        ...
        it("should returns the 400 status code if old phone number does not match", async () => {
            // arrange
            const userData = {
                fullName: "Jon Doe",
                email: "jon.doe@gmail.com",
            ...
            const sendOtpForChangeOldPhoneNumberResponse = await request(app)
                .post("/api/user/send-otp-for-change-old-phone-number")
                .set("Cookie", [`accessToken=${accessToken}`])
                .send({ phoneNumber: "1234567899", countryCode: "91" });
